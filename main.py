...
def get_args():
    ...
    # some args
    parser.add_argument('--exp_name', type = str, default='symm_vgp', help = 'experiment name')
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='override the missing-region metric nearest-neighbor count')
    parser.add_argument(
        '--resume', 
        action='store_true', 
        default=False, 
...
def main(args):
    ...
    if args.k_pred_miss is not None:
        if args.k_pred_miss <= 0:
            raise ValueError('--k_pred_miss must be positive')
        config.k_pred_miss = args.k_pred_miss
    # batch size
    if args.distributed:
